...
The **`claude` adapter is structured**: it drives the CLI in `--output-format stream-json --verbose` mode (two phases — `--permission-mode plan` for the plan-echo gate, then `acceptEdits`) and parses the JSONL event stream. This gives (a) **real turn-boundary detection** — it counts `assistant` events and hard-stops the execute phase when the tightened per-attempt turn cap (`tighten.turn_factor`) is exceeded → terminal `turn_budget_exceeded`; and (b) **real metering** — the final `result` event's `usage`/`total_cost_usd` is journaled, so driven `claude` sessions are no longer `metered: false`. The generic PTY `DrivenSession` (codex/other) is unchanged; extending structured driving to `codex exec --json` is the follow-on.

Because the daemon owns PTYs in-process, a daemon restart (the version-skew remedy) necessarily tears down every driven session — you cannot update maestro without ending in-flight driven work. Affected tasks are journaled `interrupted` (payload `reason: daemon_restart`) with a partial-diff snapshot, then terminal `failed(internal_error)`; the advisor sees them in its inbox and re-delegates if still wanted. One-shot API tasks (Tier 0) are unaffected once returned. (**v1:** on startup the daemon reconciles orphaned tasks left in-flight by a dead prior instance: active-session states → `interrupted` (payload `reason: daemon_restart` + capped partial-diff snapshot) then terminal `failed(internal_error, reason: daemon_restart)` with the worktree removed; pre-session states (`created`/`queued`) → terminal `failed` only; resting states (`verify_passed`/`blocked`/`merged`/`failed`) are never touched.)
...
**v1:** provider API keys live in the daemon's environment and are used by in-process one-shot API calls (implementer, verifier, shim); they are **stripped from driven-CLI child processes** (`env_remove` of `ANTHROPIC_API_KEY`/`ANTHROPIC_AUTH_TOKEN`/`OPENAI_API_KEY`/`CODEX_API_KEY`) so subscription CLIs use their own subscription (the `metered: false` mechanism). A driven role that sets `max_budget_usd` opts into **API billing** instead: the keys are NOT stripped (the CLI bills per-token) and the structured `claude` adapter passes `--max-budget-usd <amount>` so the CLI self-enforces a hard dollar ceiling — the meaningful cap in that mode, since a subscription is flat-rate. Per-task budgets are enforced by default at **attempt boundaries** from backend-reported token usage. The streaming credential proxy (per-response metering + mid-stream hard-stop) is built as `maestro-proxy` (a daemon-local endpoint that injects the key upstream, streams the SSE response through while metering usage into a per-task `Ledger`, and cuts the stream with a `budget_exhausted` event when the ceiling is crossed) is **on by default** in v1 (`[proxy].enabled` defaults true; opt out with `proxy.enabled = false`) and degrades gracefully to the direct path if it can't bind. The **implementer** routes gated (`X-Maestro-Task`) — a pre-forward 429 `budget_exhausted` gate between turns plus the mid-stream SSE cut — and the **verifier** routes meter-only (`X-Maestro-Meter`): its tokens accrue into the same per-task ledger (keeping the implementer's gate accurate) but it is never pre-blocked (ADR-002: verification never skipped). The shim stays direct (advisor-scoped, no task budget). Verified live: a real delegation completes through the default-on proxy across both routed surfaces.
