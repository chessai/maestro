...
7. **Scope visibility for the implementer (ADR-003/004).** The `file_allowlist` is a *post-hoc* control: it is enforced at the gate and shown to the plan-checker, but is **never injected into the implementer's own prompt**. A worker therefore can't see its scope boundary and may blindly attempt out-of-scope work — most notably a legitimate refactor that a narrow allowlist forbids (observed live: a fix task with a single-file allowlist `…/engine.rs`; the agent split the 15.7k-line file into a new `engine_body.rs`, which `commit_paths` excluded as out-of-allowlist, leaving a dangling `include!` and a confusing *build* failure rather than a clear scope signal). The allowlist already supports globs, so breadth is available (`…/src/**`) — the gaps are (a) **the worker isn't told its allowlist**, so it can't self-correct (edit-in-place vs. request-wider-scope); (b) **advisor policy**: size the allowlist glob to the task's blast radius — tight + "edit in place" for surgical fixes, directory-scoped for refactors; (c) **failure clarity**: a committed diff that references a path outside the allowlist could surface a scope-shaped error instead of a raw build break. Cheapest high-value fix is (a): render the allowlist into the implementer prompt as a visible hard boundary. Tension to preserve: tight scope is a real containment guarantee and *should* force surgical edits — the goal is right-sized, *visible* scope, not always-wider scope. **(a) + (b) LANDED** (`driven_prompt` now states the boundary + consequence + STOP-and-report escape hatch; the advisor role text shipped by `maestro init` now carries allowlist-sizing + one-task-per-delegation guidance). (c) remains open.
8. **Gate hermeticity when the task-under-test reads the host env (ADR-004).** The mechanical gate runs `check_commands` with the daemon's inherited environment. When the *repo under test is maestro itself*, its own tests read `XDG_CONFIG_HOME`/etc. — which point at the daemon's (often sandboxed/custom) config — and fail non-deterministically (observed live: `maestro-cli` `advise::tests::writable_paths_from_config` passes in a clean env but fails under a custom `XDG_CONFIG_HOME`; `cargo test` in the gate then blocks a task whose code change was fine). Workaround today: the spec's `check_commands` can scrub env (`env -u XDG_CONFIG_HOME … cargo test`). Real fix: the gate should run check_commands in a **scrubbed/hermetic env** (at least neutralize `XDG_*`/`MAESTRO_*` that leak the daemon's own config), or self-tasks should use a dedicated hermetic profile. Until then, maestro cannot reliably self-host improvement tasks through its own gate.
9. **Self-build wall-clock vs. lifetime cap.** maestro's own `cargo build`/`cargo test` from a cold worktree (inside nix) is heavy; a task that needs a retry does two cold builds and can blow a 30-min `lifetime.wall_clock_minutes` (observed: a bundled harness task `failed(lifetime_wall_clock)`). Mitigations: raise the cap for self-tasks (done ad-hoc: 60), one-task-per-delegation (smaller tasks build/verify faster). Combined with item 8 (hermeticity) and the blocked-work snapshot (now landed: `Blocked` events carry `partial_diff`), these are the "maestro can improve itself via maestro" blockers. **Build-cache options investigated:** (a) **sccache** — tried (flake `RUSTC_WRAPPER=sccache` + `CARGO_INCREMENTAL=0`) and **does not work with this nixpkgs Rust**: sccache classifies every rustc call "non-cacheable" (empty reason) — against both the nixpkgs `rustc` *wrapper* (a bash script) and the unwrapped `rustc` binary — so zero cache benefit while `CARGO_INCREMENTAL=0` slows local dev; reverted. Making sccache effective needs deeper toolchain work (e.g. a `rust-overlay`/`fenix` toolchain, or root-causing the non-cacheable classification) — its own task. (b) **shared `CARGO_TARGET_DIR` per repo** — works and is the biggest raw win, but BREAKS per-task build isolation (a task's build script can poison shared artifacts → false green in another) and cargo's target lock serializes concurrent builds; acceptable only for fully-trusted repos, so it must be opt-in + default off. (c) **keep isolated** — status quo; the 60-min cap + one-task-per-delegation are the current mitigations. Decision deferred: sccache is the *safe* method but blocked on the nix toolchain issue; shared-target is the *working* method but trades isolation.
